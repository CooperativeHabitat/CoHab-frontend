...
<template>
  <div v-if="error" class="flex items-start">
    ...
    <div>
      <h3 class="text-sm font-medium text-red-800">{{error.detail}}</h3>
      <div v-for="fieldError in error.fieldErrors" :key="fieldError.field">
        <h5 class="text-sm font-medium text-red-800">Ошибка в поле {{fieldError.field}}: {{fieldError.defaultMessage}}</h5>
      </div>
